...
function get_speaker_data($query){
      ...
      $q = $this->db->select("concat(pastors_listing.pastors_title,' ',pastors_listing.name) AS 'name' , pastors_listing.id, pastors_listing.bio, pastors_listing.photo, ts_merchant.organisation", FALSE)->from('pastors_listing')->join('ts_merchant','pastors_listing.merchant_id = ts_merchant.id','Left')->get()->result_array();
  ...
  function search_speaker($search_term){
    ...
    // $sql="SELECT * FROM $this->speakerTable WHERE name LIKE ? ";
     $sql = "SELECT concat(pastors_listing.pastors_title,'',pastosrs_listing.name) as name, pastors_listing.id pastors_listing.bio, pastors_listing.photo, ts_merchant.organisation
